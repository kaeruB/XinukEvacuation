...
object BuildingSmellPropagation {
  def calculateSmellAddends(evacuationConfig: EvacuationConfig): (CellArray, Int, Int) => Vector[Option[Signal]] = {
    def calculateSmellAddends(cells: CellArray, x: Int, y: Int): Vector[Option[Signal]] = {
      ...
      val crossWeight: Double = evacuationConfig.crossBendFactor
      val straightWeight: Double = evacuationConfig.straightBendFactor
