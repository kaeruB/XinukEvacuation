...
import pl.edu.agh.xinuk.model.{EmptyCell, GridPart, Signal, SmellingCell, WorkerId}
...
class EvacuationSimulation[ConfigType <: XinukConfig : ValueReader](
                                                                     configPrefix: String,
                                                                     metricHeaders: Vector[String],
                                                                     conflictResolver: ConflictResolver[ConfigType],
                                                                     smellPropagationFunction: EvacuationConfig => (CellArray, Int, Int) => Vector[Option[Signal]],
  ...
  implicit val config: ConfigType = {
    val evacuationConfig = rawConfig.getConfig(configPrefix)
    logger.info(WorkerActor.MetricsMarker, evacuationConfig.root().render(ConfigRenderOptions.concise()))
    logger.info(WorkerActor.MetricsMarker, logHeader)

    import net.ceedubs.ficus.Ficus._
    Try(evacuationConfig.as[ConfigType]("config")) match {
  ...
  private val workerRegionRef: ActorRef =
    ClusterSharding(system).start(
      ...
      entityProps = WorkerActor.props[ConfigType](workerRegionRef, movesControllerFactory, conflictResolver, smellPropagationFunction(config.asInstanceOf[EvacuationConfig]), emptyCellFactory),
