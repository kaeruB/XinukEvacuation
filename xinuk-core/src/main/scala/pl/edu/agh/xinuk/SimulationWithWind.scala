...
class SimulationWithWind[ConfigType <: XinukConfig : ValueReader](
       configPrefix: String,
  ...
  implicit val config: ConfigType = {
    val parsedConfig = rawConfig.getConfig(configPrefix)
    logger.info(WorkerActor.MetricsMarker, parsedConfig.root().render(ConfigRenderOptions.concise()))
